...
const analyzeSentiment = (text) => {
    ...
    URGENCY_KEYWORDS.critical.forEach(kw => { if (lower.includes(kw))
        score += 25; });
    URGENCY_KEYWORDS.high.forEach(kw => { if (lower.includes(kw))
        score += 15; });
    URGENCY_KEYWORDS.medium.forEach(kw => { if (lower.includes(kw))
        score += 5; });
    URGENCY_KEYWORDS.low.forEach(kw => { if (lower.includes(kw))
        score -= 5; });
 ...
 */
const analyzeFrequency = (category, subject) => {
    const allComplaints = storage.getComplaints();
    const similar = allComplaints.filter(c => c.category === category &&
        c.status !== 'Resolved' &&
        (c.subject.toLowerCase().split(' ').some(word => word.length > 4 && subject.toLowerCase().includes(word))));
 ...
 */
const analyzeUrgency = (category, description) => {
 ...
 */
const analyzeImpact = (description, category) => {
    const lower = description.toLowerCase();
    let score = 30;
    // Collective reports
    if (lower.includes('all student') || lower.includes('everyone'))
        score += 40;
    if (lower.includes('many student') || lower.includes('multiple'))
        score += 25;
    if (lower.includes('our class') || lower.includes('whole batch'))
        score += 20;
    // High-impact categories
    if (['Security', 'Hostel', 'Electricity', 'Transport'].includes(category))
        score += 15;
 ...
 */
const checkFairness = (category, computedScore) => {
    const allComplaints = storage.getComplaints();
 ...
 */
export const mlPrioritize = async (category, subject, description, isAbusive) => {
    ...
    const finalScore = (urgencyScore * 0.35 + // Urgency is most important
